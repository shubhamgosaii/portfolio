...
interface NavbarLink { id: string; label: string; }
...
export default function Navbar({ dark, setDark }: NavbarProps) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(navRef, (s) => {
  ...
  const renderHover3D = (label: string) => (
    <motion.span
      style={{ rotateX: xSpring, rotateY: ySpring, display: 'inline-block', cursor: 'pointer' }}
  ...
  return (
    <>
      ...
      >
        {!showMenuButton && (
          <nav className="hidden md:flex gap-8 items-center">
            {links.map((l) => (
              <a key={l.id} href={"#" + l.id} className="px-2 py-1 font-semibold capitalize" style={{ color: textColor }}>
                {renderHover3D(l.label)}
              </a>
            ))}
            <button onClick={() => setDark(!dark)} className="ml-6 p-2 rounded-full shadow-lg" style={{ backgroundColor: bgColor, color: textColor }}>
        ...
        {showMenuButton && (
          <button onClick={() => setMenuOpen(!menuOpen)} className="ml-4 md:ml-0 p-2 rounded-full shadow-lg" style={{ color: textColor }}>
            <Menu size={28} />
          </button>
        )}
      </motion.header>
      ...
      {/* Mobile Menu */}
      {menuOpen && (
        <div ref={menuRef} className="fixed top-0 right-0 h-full w-64 backdrop-blur-lg shadow-xl z-50 p-6 flex flex-col transition-colors duration-300" style={{ backgroundColor: bgColor, color: textColor }}>
          <button onClick={() => setMenuOpen(false)} className="self-end mb-6">
            <X size={28} />
          </button>
          <div className="flex flex-col gap-6 mt-4">
            {links.map((l) => (
              <a key={l.id} href={"#" + l.id} onClick={() => setMenuOpen(false)} className="text-lg font-medium transition" style={{ color: textColor }}>
                {renderHover3D(l.label)}
              </a>
            ))}
            <button onClick={() => setDark(!dark)} className="mt-6 flex items-center gap-2 px-4 py-2 rounded-lg shadow-lg" style={{ backgroundColor: bgColor, color: textColor }}>
