...
export default function Shubham({ dark }: { dark: boolean }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(shubhamRef, (s) => {
      if (s.exists()) setShubham(s.val());
      setLoading(false);
    });
    return () => unsubscribe();
  }, []);
  ...
  if (loading)
    return (
      <section className="section min-h-screen flex items-center justify-center">
        <img src="https://i.gifer.com/ZZ5H.gif" alt="" className="w-16 h-16 sm:w-12 sm:h-12" />
      </section>
    );
  ...
  const renderHoverWords = (text: string) =>
    text.split(' ').map((word, i) => (
      <motion.span
        ...
        onMouseLeave={(e) => {
          const target = e.target as HTMLElement;
          target.style.color = shubham?.textColor || (dark ? '#FFFFFF' : '#111827');
        }}
        whileHover={{
          scale: 1.3,
  ...
  return (
    <motion.section
      ...
      style={{
        backgroundImage: shubham?.backgroundImage ? `url(${shubham.backgroundImage})` : undefined,
    ...
    >
      <div
        ...
        style={{
          backgroundColor: shubham?.overlayColor || (dark ? 'rgba(0,0,0,0.6)' : 'rgba(0,0,0,0.3)'),
        }}
      ></div>
